   ...
   return (
      <div className="flex flex-col gap-4">
         <div className="flex items-center justify-between">
            <button onClick={() => onChangeViewDate(new Date(year, month - 1, 1))} className="w-8 h-8 rounded-xl border border-base-200 flex items-center justify-center opacity-50 hover:opacity-100 hover:bg-base-200 transition-all duration-200">
               <FiArrowLeft size={14} />
            </button>
         ...
         <div className="grid grid-cols-7 gap-1">
            ...
            {cells.map((day, i) => {
               ...
               return (
                  <button key={i} disabled={isDisabled} onClick={() => handleClick(day)}
...
function AddAttendanceWizard({
   ...
   // дата
   viewDate, onChangeViewDate, allBusyDates,
   ...
   const goNext = async () => {
      ...
      if (step === 1) {
         if (!date) { setError('Tarix seçin'); return; }
         setStep(2);
         return;
      }
      ...
      if (step === 2) {
         if (!titleForm.title.trim() || !String(titleForm.hour).trim() || !titleForm.type) {
            setError('Bütün sahələri doldurun'); return;
         }
         try {
            setBusy(true);
            if (!titleId) {
               ...
               setTitleId(newId);
               setTitleDirty(false);
               onDraftChange?.({ step: 3, date, titleForm, titleId: newId, attStudents });
            } else if (titleDirty) {
      ...
      <div className="modal modal-open z-50" role="dialog">
         <div className="modal-box rounded-2xl border border-base-200 shadow-xl flex flex-col p-0 max-w-lg overflow-hidden">
            ...
            <div className="px-8 pb-2 flex flex-col gap-3 min-h-[280px]">
               ...
               {step === 1 && (
                  <CalendarStep
                     ...
                     selectedDate={date}
                     onPickDay={(d, m) => { setDate(d); setPickedMonth(m); }}
                  />
               )}
               ...
               {step === 2 && (
                  <div className="flex flex-col gap-4">
                     <div className="flex flex-col gap-1">
                        ...
                        <input type="text" value={titleForm.title} onChange={e => updateTitleField('title', e.target.value)}
                     ...
                     <div className="flex flex-col gap-1">
                        ...
                        <input type="number" min={1} max={7} value={titleForm.hour} onChange={e => updateTitleField('hour', e.target.value)}
                           className="input w-full pl-4 pr-4 py-2.5 rounded-xl border border-base-200 bg-base-200/50 focus:outline-none focus:border-[#8B5CF6] transition-all duration-200 text-sm"
                     ...
                     <div className="flex flex-col gap-2">
                        ...
                        <div className="flex gap-3">
                           <button type="button" onClick={() => updateTitleField('type', 'N')}
                           ...
                           <button type="button" onClick={() => updateTitleField('type', 'P')}
                        ...
                        {students.map((student, index) => {
                           ...
                           return (
                              <div key={sid} className="flex items-center gap-3 px-4 py-2.5 rounded-xl border border-base-200 hover:bg-base-200/30 transition-all duration-200">
                                 ...
                                 <button onClick={() => toggleStudent(sid)}
               ...
               {step === 4 && (
                  <div className="flex flex-col gap-4">
                     <div className="rounded-2xl border border-base-200 overflow-hidden">
                        <div className="bg-gradient-to-r from-[#8B5CF6] to-[#3B82F6] px-5 py-4 text-white">
                           ...
                           <div className="flex items-center gap-3 mt-1 text-xs opacity-90">
                              ...
                              <span className="flex items-center gap-1"><FiClock size={11} />{titleForm.hour} saat</span>
                           </div>
                        </div>
                        <div className="p-5 flex flex-col gap-3">
                           <div className="flex items-center gap-2 text-xs opacity-60">
                              <FiUsers size={12} /> {groupName} · {subjectName}
                           </div>
                     ...
                     <button onClick={() => setStep(2)} className="text-xs text-[#8B5CF6] opacity-70 hover:opacity-100 transition-opacity self-start">
                        Mövzunu redaktə et
                     </button>
                  </div>
               )}
               ...
               {error && (
                  <span className="text-red-400 text-xs flex items-center gap-1 mt-1">{error}</span>
               )}
            </div>
            ...
            <div className="px-8 pt-4 pb-7 flex flex-col gap-5 mt-auto">
               <div className="flex gap-3">
                  {step > 1 && (
                     <button onClick={goBack} disabled={busy} className="px-5 py-2.5 rounded-xl border border-base-200 bg-base-200/50 text-sm font-semibold hover:bg-base-200 transition-all duration-200 disabled:opacity-60">
                        Geri
                     </button>
                  )}
                  {step < 4 && (
                  ...
                  {step === 4 && (
                     <button onClick={handleConfirm} disabled={busy}
                        className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-[#8B5CF6] to-[#3B82F6] text-white font-semibold text-sm flex items-center justify-center gap-2 shadow-md hover:shadow-lg hover:opacity-90 transition-all duration-200 disabled:opacity-60">
                        {busy ? <span className="loading loading-spinner loading-xs" /> : <><FiCheck size={15} /> Təsdiqlə</>}
                     </button>
                  )}
               </div>
