...
function EditAttendanceDayModal({ dayDoc, students, onSave, onClose, saving, error }) {
  ...
  const initial = useMemo(() => {
    const map = {};
    students.forEach((s) => {
      const found = dayDoc.students?.find((x) => (x.student?._id ?? x.student)?.toString() === s._id?.toString());
          ...
          {confirmClose && (
            <div className="flex flex-col gap-3 p-4 rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20">
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col gap-0.5">
                  ...
                  <span className="text-xs text-amber-600 dark:text-amber-500 opacity-80">
                    Dəyişikliklər saxlanılmayıb. Çıxmaq istəyirsiniz?
                  </span>
                </div>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={onClose}
                  className="flex-1 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold transition-all duration-200"
                >
                  Bəli, çıx
                </button>
                <button
