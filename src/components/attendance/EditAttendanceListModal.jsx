...
function EditAttendanceListModal({
   availableMonths, selectedMonth, onSelectMonth,
   attendanceData, attendanceLoading, onEditDay, onClose,
}) {
               ...
               {availableMonths.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                     {availableMonths.map(m => (
                        <button key={`${m.month}-${m.year}`} onClick={() => onSelectMonth(m)}
               ...
               {attendanceLoading ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                     {sortedDays.map((doc, index) => {
                        ...
                        return (
                           <div key={doc._id} className="bg-base-100 border border-base-200 rounded-2xl shadow-sm p-4 flex items-center gap-4 hover:shadow-md transition-all duration-200">
                              <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${colors[index % colors.length]} flex flex-col items-center justify-center text-white shadow-md shrink-0`}>
                              ...
                              <div className="flex-1 min-w-0">
                                 ...
                                 <div className="text-xs opacity-50 mt-0.5">
                                    Ümumi: {total} · İştirak: <span className="text-emerald-500 font-semibold">{present} edib</span> / <span className="text-red-400 font-semibold">{absent} etməyib</span>
                                 </div>
                              </div>
                              <button onClick={() => onEditDay(doc)}
