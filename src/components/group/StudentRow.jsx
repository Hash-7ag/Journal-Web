   ...
   return (
      <div key={sid ?? index} className="bg-base-100 border border-base-200 rounded-2xl shadow-sm px-5 py-4 flex items-center gap-4 hover:shadow-md transition-all duration-200">
         ...
         <div className="hidden md:flex items-center gap-4 shrink-0">
            {s.email && <div className="flex items-center gap-1.5 text-xs opacity-40"><FiMail size={12} />{s.email}</div>}
            {s.phoneNumber && <div className="flex items-center gap-1.5 text-xs opacity-40"><FiPhone size={12} />{s.phoneNumber}</div>}
         </div>
         <button
            onClick={() => onInfo(s)}
