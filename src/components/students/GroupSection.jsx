...
const groupColors = ['from-[#8B5CF6] to-[#3B82F6]', 'from-[#3B82F6] to-[#60A5FA]', 'from-[#8B5CF6] to-[#A78BFA]', 'from-[#6366F1] to-[#8B5CF6]', 'from-[#3B82F6] to-[#8B5CF6]', 'from-[#A78BFA] to-[#60A5FA]'];
      ...
      <div className="bg-base-100 border border-base-200 rounded-2xl shadow-sm overflow-hidden">
         <button onClick={() => setCollapsed(p => !p)}
            ...
            <div className="flex items-center gap-3 shrink-0">
               <div className="flex items-center gap-1.5 text-xs opacity-40"><FiUsers size={13} />{total} şagird</div>
         ...
         {!collapsed && (
            <div className="p-4 flex flex-col gap-2">
               {loading ? (
                  <div className="flex justify-center py-8"><span className="loading loading-spinner loading-md" style={{ color: '#8B5CF6' }} /></div>
               ) : students.length === 0 ? (
                  ...
                  <>
                     {students.map((student, i) => {
                        ...
                        return (
                           <div key={student._id} className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-base-200/40 hover:bg-base-200/70 transition-all duration-200">
                              ...
                              <div className="hidden md:flex items-center gap-4 shrink-0">
                                 {student.email && <div className="flex items-center gap-1.5 text-xs opacity-40"><FiMail size={11} />{student.email}</div>}
                                 {student.phoneNumber && <div className="flex items-center gap-1.5 text-xs opacity-40"><FiPhone size={11} />{student.phoneNumber}</div>}
                              </div>
                              <button onClick={() => onEdit(student)} className="p-2 rounded-xl border border-base-200 opacity-40 hover:opacity-70 hover:bg-base-200 transition-all duration-200 shrink-0">
                              ...
                              <button onClick={() => onInfo(student)} className="p-2 rounded-xl border border-base-200 opacity-40 hover:opacity-70 hover:bg-base-200 transition-all duration-200 shrink-0">
                                 <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" stroke="currentColor" fill="none" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round">
