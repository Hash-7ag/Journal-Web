...
function SubjectGradeModal({ item, grade, color, totalColor, onClose }) {
  ...
  const total = grade
    ? (grade.col1?.grade?.grade ?? 0) +
      (grade.col2?.grade?.grade ?? 0) +
      (grade.coursework?.grade?.grade ?? 0) +
      (grade.attendence?.grade?.grade ?? 0) +
        ...
        <div className="p-6 flex flex-col gap-5">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              <div
                className={`w-11 h-11 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center text-white font-bold text-base shadow-md shrink-0`}
              >
          ...
          <div className="bg-base-200/50 rounded-xl p-4 border border-base-200">
            ...
            <GradeRow
              label="Davamiyyət"
              value={grade?.attendence?.grade?.grade}
              max={10}
              color={grade?.attendence?.limited ? 'text-red-400' : 'text-emerald-400'}
            />
          ...
          <div className="flex items-center justify-between px-1">
            ...
            <div className="flex items-baseline gap-1">
              <span className={`text-3xl font-bold ${totalColor(calcedTotal)}`}>
                {calcedTotal != null ? calcedTotal : '—'}
              </span>
