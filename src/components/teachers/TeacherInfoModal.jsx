                ...
                groups.map((group, gi) => {
                  const mySubjects =
                    group.subjects?.filter(
                      (item) =>
                        item.teacher?.toString() === teacher._id || item.teacher?._id?.toString() === teacher._id,
                    ) ?? [];
                  return (
                    <div key={group._id} className="bg-base-100 border border-base-200 rounded-xl overflow-hidden">
                      <div className="flex items-center gap-3 px-4 py-3 border-b border-base-200">
                        <div
                          className={`w-9 h-9 rounded-lg bg-gradient-to-br ${groupColors[gi % groupColors.length]} flex items-center justify-center text-white font-bold text-sm shadow-sm shrink-0`}
                        >
                          {group.groupNumber}
                        </div>
                        ...
                        {mySubjects.length === 0 ? (
                          ...
                          mySubjects.map((item, si) => {
                            ...
                            return (
                              <div
                                key={subject._id ?? si}
                                className="flex items-center gap-3 px-3 py-2 rounded-lg border border-base-200 bg-base-200/30"
                              >
                                <div
                                  className={`w-7 h-7 rounded-lg bg-gradient-to-br ${subColors[si % subColors.length]} flex items-center justify-center text-white shrink-0`}
                                >
                                  <FiBook size={12} />
                                </div>
                                <div className="flex-1 min-w-0">
