...
function ResetPasswordBlock({ id, role }) {
   ...
   const handleReset = async () => {
      try {
         setLoading(true); setError(''); setNewPassword(''); setCopied(false); setConfirm(false);
         const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789!@#';
         let pwd = '';
         for (let i = 0; i < 10; i++) pwd += chars[Math.floor(Math.random() * chars.length)];
         const endpoint = role === 'teacher' ? `/admin/resetTeacherPassword/${id}` : `/admin/resetStudentPassword/${id}`;
            ...
            <div className="flex flex-col gap-3 p-4 rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20">
               <div className="flex items-start gap-2">
                  ...
                  <div className="flex flex-col gap-0.5">
                     ...
                     <span className="text-xs text-amber-600 dark:text-amber-500 opacity-80">Parolu sıfırlasanız bu istifadəçi bütün cihazlardan çıxarılacaq.</span>
                  </div>
               </div>
               <div className="flex gap-2">
                  <button onClick={handleReset} disabled={loading}
                     ...
                     {loading ? <><span className="loading loading-spinner loading-xs" /> Sıfırlanır...</> : 'Bəli, sıfırla'}
                  </button>
