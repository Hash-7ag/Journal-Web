...
function GroupDetail() {
  ...
  const handleAddStudents = async () => {
    ...
    try {
      ...
      setModalError('');
      await Promise.all(
        selectedStudents.map((studentId) => api.patch('/admin/addStudentToGroup', { groupId: id, studentId })),
      );
          ...
          <div className="flex-1 flex flex-col gap-3 w-full">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              <InfoCard
                ...
                label="Şagird / Fən"
                value={`${students.length} / ${subjects.length}`}
              />
            </div>
          </div>
        </div>
            ...
            filteredStudents.map((s, index) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border text-left transition-all duration-200 ${isSelected ? 'border-[#8B5CF6]/40 bg-gradient-to-r from-[#8B5CF6]/10 to-[#3B82F6]/10 shadow-sm' : 'border-base-200 hover:border-base-300 hover:bg-base-200/40'}`}
                >
                  <div
                    className={`w-9 h-9 rounded-xl flex items-center justify-center text-white font-bold text-sm shadow-sm shrink-0 transition-all duration-200 ${isSelected ? 'bg-gradient-to-br from-[#8B5CF6] to-[#3B82F6]' : `bg-gradient-to-br ${colors[index % colors.length]}`}`}
                  >
