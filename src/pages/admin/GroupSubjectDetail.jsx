...
function GroupSubjectDetail() {
   ...
   const handleGradeEdit = async () => {
      ...
      if (Number(gradeEditValue) < 0 || Number(gradeEditValue) > max) { setGradeEditError(`0 - ${max} aralığında olmalıdır`); return; }
      try {
         setGradeEditSubmitting(true); setGradeEditError('');
         await api.patch(gradeEndpoints[gradeEdit.type](id, subjectId, gradeEdit.studentId), { grade: Number(gradeEditValue) });
         const gradesRes = await api.get(`/admin/getGrades/${id}/${subjectId}`);
         setGrades(gradesRes.data); setGradeEdit(null);
      } catch (err) { setGradeEditError(err.response?.data?.message || 'Xəta baş verdi'); }
      finally { setGradeEditSubmitting(false); }
   };
   ...
   const getGrade = (studentId, type) => {
      ...
      return (map[type] ?? []).find(g => g.student?.toString() === studentId?.toString() || g.student === studentId)?.grade?.grade ?? null;
   };
         ...
         <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1 flex flex-col gap-3">
               <div className="flex gap-2 mb-2">
                  ...
                  <TabBtn active={activeTab === 'attendance'} onClick={handleAttendanceTab} icon={<FiUser size={14} />} label="Davamiyyət" />
               </div>
               ...
               {activeTab === 'grades' && (
                  <>
                     {students.length === 0 ? (
                        ...
                        const isLimited = (grades?.attendenceGrades ?? []).find(g => g.student?.toString() === sid?.toString())?.limited;
                        return (
                           <div key={sid} className={`bg-base-100 border rounded-xl shadow-sm px-4 py-3 flex flex-wrap items-center gap-3 hover:shadow-md transition-all duration-200 ${isLimited ? 'border-red-300 dark:border-red-800 bg-red-50/50 dark:bg-red-900/10' : 'border-base-200'}`}>
                              ...
                              <div className="flex-1 flex flex-wrap items-center gap-3 lg:gap-5">
                                 {gradeColumns.map(col => (
                                    <GradeCell key={col.key} label={col.label} value={getGrade(sid, col.key)} max={col.max}
                                       color={col.key === 'attendence' && isLimited ? 'text-red-400' : col.color}
                                       editable={col.key !== 'attendence' && getGrade(sid, col.key) != null}
                                       onEdit={() => { setGradeEdit({ studentId: sid, type: col.key }); setGradeEditValue(String(getGrade(sid, col.key))); setGradeEditError(''); }}
                                    />
                                 ))}
                              </div>
                              <div className="w-px h-8 bg-base-200 shrink-0 hidden lg:block" />
