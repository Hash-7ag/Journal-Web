...
const studentColors = ['from-[#8B5CF6] to-[#3B82F6]', 'from-[#3B82F6] to-[#60A5FA]', 'from-[#8B5CF6] to-[#A78BFA]', 'from-[#6366F1] to-[#8B5CF6]', 'from-[#A78BFA] to-[#60A5FA]', 'from-[#3B82F6] to-[#8B5CF6]'];
   ...
   return (
      <div className="bg-base-100 border border-base-200 rounded-2xl shadow-sm px-5 py-4 flex items-center gap-4 hover:shadow-md transition-all duration-200">
         ...
         <div className="hidden md:flex items-center gap-4 shrink-0">
            {student.email && <div className="flex items-center gap-1.5 text-xs opacity-40"><FiMail size={12} />{student.email}</div>}
            {student.phoneNumber && <div className="flex items-center gap-1.5 text-xs opacity-40"><FiPhone size={12} />{student.phoneNumber}</div>}
         </div>
         <button onClick={() => onEdit(student)} className="p-2 rounded-xl border border-base-200 opacity-40 hover:opacity-70 hover:bg-base-200 transition-all duration-200">
         ...
         <button onClick={() => onInfo(student)} className="p-2 rounded-xl border border-base-200 opacity-40 hover:opacity-70 hover:bg-base-200 transition-all duration-200">
...
function Students() {
   ...
   const openEditStudent = (student) => {
      setEditForm({
         ...
         fatherName: student.fatherName ?? '', username: student.username ?? '',
         email: student.email ?? '', phone: student.phoneNumber?.replace('+994', '') ?? '',
      });
      setEditError(''); setEditModal(student);
   };
   ...
   const handleEditStudent = async () => {
      try {
         ...
         fetchFreeStudents(freePage); setEditModal(null);
   ...
   const fetchFreeStudents = async (p = 1) => {
      try {
         ...
         setFreeTotal(total);
         if (!initialTabSet) { setActiveTab(total === 0 ? 'assigned' : 'free'); setInitialTabSet(true); }
   ...
   const handleAddStudent = async () => {
      ...
      if (missing.length) { setModalError(`Zəhmət olmasa doldurun: ${missing.join(', ')}`); return; }
      try {
         setSubmitting(true);
         ...
         setShowPassword(false); setModalError('');
      ...
      <div className="min-h-[calc(100vh-4rem)] px-6 py-8">
         <div className="flex justify-between items-center mb-8">
            <div>
               ...
               <p className="text-xs opacity-40 mt-0.5">
                  {activeTab === 'free' ? `${freeTotal} boş şagird · Səhifə ${freePage} / ${freeTotalPages}` : `${groups.length} qrup`}
               </p>
            </div>
            <button onClick={() => { setIsModalOpen(true); setShowPassword(false); setModalError(''); }}
         ...
         <div className="flex gap-2 mb-6">
            {[
               ...
               <TabBtn key={tab.key} active={activeTab === tab.key} onClick={() => setActiveTab(tab.key)} icon={tab.icon} label={tab.label} count={tab.count} />
            ))}
         </div>
         ...
         {activeTab === 'free' && (
            <>
               {freeLoading ? (
                  <div className="flex justify-center py-20"><span className="loading loading-spinner loading-lg" style={{ color: '#8B5CF6' }} /></div>
                  ...
                  <div className="flex flex-col gap-3">
                     {freeStudents.map((student, index) => (
                        <StudentRow key={student._id} student={student} globalIndex={(freePage - 1) * pageSize + index} onInfo={setInfoModal} onEdit={openEditStudent} />
                     ))}
                  </div>
               )}
               ...
               {groupsLoading ? (
                  ...
                  <div className="flex flex-col gap-4">
                     {groups.map((group, index) => (
                        <GroupSection key={group._id} group={group} index={index} onInfo={setInfoModal} onEdit={openEditStudent} />
                     ))}
                  </div>
               )}
            </>
            ...
            <div className="modal modal-open z-40" role="dialog">
               <div className="modal-box rounded-2xl border border-base-200 shadow-xl flex flex-col gap-5 p-8 max-w-lg">
                  ...
                  <div className="flex gap-3 pt-1">
                     ...
                     <button onClick={() => { setIsModalOpen(false); setModalError(''); setShowPassword(false); }}
