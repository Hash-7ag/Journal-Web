      ...
      <div className="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center px-4">
         ...
         <div className="text-center mb-12">
            ...
            <p className="text-base opacity-50 max-w-sm mx-auto leading-relaxed">
               Kollecimizin Elektron Cədvəl Proqramına xoş gəlmisiniz!
               Hesabınıza daxil olmaq üçün rolunuzu seçin.
            </p>
         </div>
         ...
         <div className="flex flex-col sm:flex-row gap-4 w-full max-w-2xl">
            {roles.map(({ key, label, desc, icon, color }) => (
               ...
               >
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center text-white mb-4 shadow-md group-hover:scale-110 transition-transform duration-300`}>
                     {icon}
                  </div>
                  <div className="font-semibold text-base mb-1">{label}</div>
                  <div className="text-xs opacity-50 leading-relaxed mb-4">{desc}</div>
                  <div className={`flex items-center gap-1 text-xs font-medium bg-gradient-to-r ${color} bg-clip-text text-transparent`}>
