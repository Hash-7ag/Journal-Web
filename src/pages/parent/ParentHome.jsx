...
function ParentHome() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await api.get('/parent/getMyProfile');
  ...
  // модалка подтверждения email — раз за сессию, если не подтверждён
  useEffect(() => {
    if (!userData) return;
  ...
  const handleEditSubmit = async () => {
    try {
      setSubmitting(true);
      setEditError('');
      // updateMyInfo родителя требует email и phone — email шлём текущий (не меняется здесь)
      await api.patch('/parent/updateMyInfo', {
        email: userData.email,
        phone: phoneToRaw(editForm.phone),
      });
      await fetchData();
      setEditModal(false);
    } catch (err) {
  ...
  const infoFields = [
    ...
    { icon: <FiMail size={16} />, label: 'Email', value: userData.email },
    {
      ...
      label: 'Övladlar',
      value: children.map((c) => `${c.name} ${c.surname}`).join(', ') || '—',
    },
  ];
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-10">
      <ProfileCard
        userData={{ ...userData, roleLabel: 'Valideyn' }}
        initials={initials}
