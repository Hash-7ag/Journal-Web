...
function StudentGrades() {
   ...
   // загрузка предметов + оценок для семестра
   const fetchSubjectsAndGrades = async (gid, semestr = null) => {
      try {
         setSubjectsLoading(true);
         const url = semestr != null
            ? `/student/getMySubjects/${gid}?semestr=${semestr}`
            : `/student/getMySubjects/${gid}`;
         const subjectsRes = await api.get(url);
   ...
   useEffect(() => {
      const init = async () => {
         try {
            ...
            // список семестров + предметы текущего семестра
            const [semestrsRes] = await Promise.all([
               api.get(`/student/getGroupSemestrs/${gid}`),
               fetchSubjectsAndGrades(gid),
            ]);
            setSemestrs(semestrsRes.data.semestrs ?? []);
         } catch (err) {
            setError(err.response?.data?.message || err.message || 'Yükləmə xətası');
         } finally {
            setLoading(false);
   ...
   const handleSelectSemestr = (semestr) => {
      if (!groupId) return;
      fetchSubjectsAndGrades(groupId, semestr === currentSemestr ? null : semestr);
   };
