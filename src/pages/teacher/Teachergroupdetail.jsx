...
const parseMonthStr = (str) => { const [mm, yyyy] = str.split('-'); return { month: Number(mm), year: Number(yyyy) }; };
...
const gradeTypeEndpoint = {
   collegium1: '/teacher/createCollegium1', collegium2: '/teacher/createCollegium2',
   coursework: '/teacher/createCoursework', exam: '/teacher/createExam',
};
const gradeTypeLabel = { collegium1: 'Kollegium 1', collegium2: 'Kollegium 2', coursework: 'Kurswork', exam: 'İmtahan' };
...
function TeacherGroupDetail() {
   ...
   useEffect(() => {
      ...
      if (!el) return;
      const handleWheel = (e) => { if (Math.abs(e.deltaY) > Math.abs(e.deltaX)) { el.scrollLeft += e.deltaY; e.preventDefault(); } };
   ...
   const fetchAll = async () => {
      try {
         ...
         const allStudents = (studentsRes.data ?? []).flatMap(g => g.students ?? []).map(s => s.student ?? s).filter(s => s && s._id);
         setStudents(allStudents);
         setGrades(gradesRes.data);
         const initMap = {};
         allStudents.forEach(s => { initMap[s._id] = true; });
         setNewAttStudents(initMap);
      } catch (err) { setError(err.response?.data?.message || err.message || 'Yükləmə xətası'); }
   ...
   const fetchAttendance = async (month, year) => {
      try {
         ...
         const res = await api.get(`/teacher/getAttendence/${group}/${subject}?date=${String(month).padStart(2, '0')}-${year}`);
         setAttendanceData(res.data ?? []);
      } catch (err) { if (err.response?.status === 404) setAttendanceData([]); }
   ...
   const buildTable = () => {
      ...
      return { days: Object.keys(dayMap).map(Number).sort((a, b) => a - b), dayMap };
   };

   const getAttendenceForStudent = (dayDoc, studentId) => {
      if (!dayDoc) return null;
      return dayDoc.students?.find(s => s.student?.toString() === studentId?.toString() || s.student === studentId)?.attendence ?? null;
   };
   ...
   const getStudentGrade = (studentId, type) => {
      ...
      const map = {
         collegium1: grades.collegium1Grades, collegium2: grades.collegium2Grades,
         attendence: grades.attendenceGrades, coursework: grades.courseworkGrades, exam: grades.examGrades,
      };
      return (map[type] ?? []).find(g => g.student?.toString() === studentId?.toString() || g.student === studentId);
   };

   const calcTotal = (studentId) => {
      const vals = ['collegium1', 'collegium2', 'coursework', 'attendence', 'exam'].map(t => getStudentGrade(studentId, t)?.grade?.grade ?? 0);
   ...
   const openGradeModal = (type, studentId) => {
      setGradeType(type); setSelectedStudent(studentId);
      setGradeValue(''); setModalError(''); setGradeModal(true);
   };
   ...
   const handleSubmitGrade = async () => {
      ...
      try {
         ...
         await api.post(gradeTypeEndpoint[gradeType], { student: selectedStudent, subject, group, grade: { grade: String(gradeValue), date: gradeDate } });
         setGradeModal(false);
         await fetchAll();
   ...
   return (
      <div className="min-h-[calc(100vh-4rem)] px-6 py-8">
         ...
         <div className="flex gap-2 mb-6">
            <TabBtn active={activeTab === 'grades'} onClick={() => setActiveTab('grades')} icon={<FiBook size={14} />} label="Qiymətlər" />
            <TabBtn active={activeTab === 'attendance'} onClick={handleAttendanceTab} icon={<PiStudent size={14} />} label="Qayıblar" />
         </div>
                  ...
                  return (
                     <div key={sid} className={`bg-base-100 border rounded-xl shadow-sm px-4 py-3 flex flex-wrap items-center gap-2 hover:shadow-md transition-all duration-200 ${isLimited ? 'border-red-300 dark:border-red-800 bg-red-50/50 dark:bg-red-900/10' : 'border-base-200'}`}>
                        ...
                        <div className="w-full flex flex-wrap items-center gap-4">
                           ...
                           <button onClick={() => setStudentModal(student)} className="w-7 h-7 rounded-lg border border-base-200 flex items-center justify-center opacity-40 hover:opacity-100 hover:bg-base-200 transition-all duration-200 shrink-0">
         ...
         {activeTab === 'attendance' && (
            <div className="flex flex-col gap-5">
               {availableMonths.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                     {availableMonths.map(m => (
                        <button key={`${m.month}-${m.year}`} onClick={() => { setSelectedMonth(m); fetchAttendance(m.month, m.year); }}
               ...
               <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => { setCalendarViewDate(new Date()); setCalendarModal(true); setAttError(''); setAttSuccess(''); }}
               ...
               {attendanceLoading ? (
                  <div className="flex justify-center py-10"><span className="loading loading-spinner loading-md" style={{ color: '#8B5CF6' }} /></div>
               ) : days.length > 0 ? (
                  <div ref={tableRef} className="overflow-x-auto rounded-2xl border border-base-200 shadow-sm w-full p-4" style={{ maxWidth: '100%' }}>
                     <table className="text-sm" style={{ borderCollapse: 'separate', borderSpacing: 0, width: 'max-content' }}>
                        <thead>
                           <tr className="border-b border-base-200 bg-base-100 sticky top-0 z-[5]">
                              <th className="sticky left-0 top-0 z-20 bg-base-100 text-left px-4 py-3 font-semibold text-xs opacity-50 min-w-[180px] border-r border-base-200">Şagird</th>
                              {days.map(day => (
                                 ...
                                 <tr key={sid} className="border-b border-base-200 last:border-0 bg-base-100 hover:bg-base-200/30 transition-colors">
                                    <td className="sticky left-0 z-10 bg-base-100 px-4 py-2.5 border-r border-base-200">
                                       <div className="flex items-center justify-between">
                                          ...
                                          <button onClick={() => setStudentModal(student)} className="w-8 h-8 rounded-xl border border-base-200 flex items-center justify-center opacity-40 hover:opacity-100 hover:bg-base-200 transition-all duration-200 shrink-0">
                                             <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" stroke="currentColor" fill="none" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round">
                                                <circle cx="12" cy="12" r="10" /><line x1="12" y1="16" x2="12" y2="12" /><line x1="12" y1="8" x2="12.01" y2="8" />
                                             </svg>
                                          </button>
                                    ...
                                    {days.map(day => {
                                       ...
                                       return (
                                          <td key={day} className="py-2.5 text-center w-10">
                                             {att === null ? <span className="text-xs opacity-20">—</span>
                                                : att === true ? <span className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-emerald-500 text-white text-xs font-bold shadow-sm">+</span>
            ...
            <div className="modal modal-open" role="dialog">
               <div className="modal-box rounded-2xl border border-base-200 shadow-xl flex flex-col gap-5 p-8 max-w-sm">
                  ...
                  <div className="flex gap-3 pt-1">
                     ...
                     <button onClick={() => setGradeModal(false)} className="flex-1 py-2.5 rounded-xl border border-base-200 bg-base-200/50 text-sm font-semibold hover:bg-base-200 transition-all duration-200">
                        Ləğv et
                     </button>
                  </div>
               </div>
         ...
         {attModal && (
            <AttendanceModal
               date={newAttDate} students={students} attStudents={newAttStudents}
               onToggle={sid => setNewAttStudents(prev => ({ ...prev, [sid]: !prev[sid] }))}
               onSave={async () => { await handleSaveAttendance(); setAttModal(false); }}
               onClose={() => { setAttModal(false); setAttError(''); }}
